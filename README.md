...
void titleDraw(void);
int menuDraw(void);
int bomb(char(*tMap)[40], int* pX, int* pY, int _x, int _y, int* pKey);
void infoDraw(void);
...
void gLoop(void)
{
	...
	while (playing)
	{
		...
		case SUBMIT:
			playing = 0;
		case BUP:
			playing = bomb(tempMap, &pX, &pY, 0, -1, &pKey);
			break;
		case BDOWN:
			playing = bomb(tempMap, &pX, &pY, 0, 1, &pKey);
			break;
		case BRIGHT:
			playing = bomb(tempMap, &pX, &pY, 1, 0, &pKey);
			break;
		case BLEFT:
			playing = bomb(tempMap, &pX, &pY, -1, 0, &pKey);
			break;
